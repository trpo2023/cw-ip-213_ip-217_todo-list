...
function removeTask(item, id, side) {
  ...
  !side.querySelectorAll('li').length &&
    side
      .querySelector('.todo_task-container_list-wrap-no-tasks')
      .classList.remove('todo_task-container_list-wrap-no-tasks-off');
}
...
function addTask(task, side) {
  !side.querySelectorAll('li').length &&
    side
      .querySelector('.todo_task-container_list-wrap-no-tasks')
      .classList.add('todo_task-container_list-wrap-no-tasks-off');
  // wrapper.push(task);
...
const taskItemComponent = (task, side) => {
  ...
  taskItem.classList.add(
    !side.parentElement.classList.contains('todo_task-container_list-wrap')
      ? 'todo_task-container_completed-list-wrap-element'
      : 'o'
  );
...
function setTasks() {
  Object.values(localStorage)
    ...
    .map((el) => addTask(el, taskList, activeTodo));
  Object.values(localStorage)
    .map((el) => JSON.parse(el))
    .filter((el) => el.completed === true)
    .map((el) => addTask(el, completedTaskList, completedTodo));
}
setTasks();
